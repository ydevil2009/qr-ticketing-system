...
const CLOUDINARY_FOLDER = process.env.CLOUDINARY_FOLDER || 'ticket-payment-screenshots';
...
async function createTicketPdfBuffer({ eventName, passLabel, name, email, contact, passType, id, verifyUrl }) {
      ...
      <body style="margin:0;background:#000;color:#fff;font-family:Arial,sans-serif;min-height:100vh;display:flex;justify-content:center;">
        <div style="width:100%;max-width:980px;padding:40px 20px 70px 20px;text-align:center;">
          ...
          <form method="POST" action="/generate" enctype="multipart/form-data"
            ...
            <input name="name" required style="width:100%;padding:18px 20px;margin:0 0 30px 0;border-radius:6px;border:1px solid #777;font-size:22px;box-sizing:border-box;">

            <label style="display:block;font-size:32px;margin-bottom:14px;">✉️ EMAIL:-</label>
            <input name="email" type="email" required style="width:100%;padding:18px 20px;margin:0 0 30px 0;border-radius:6px;border:1px solid #777;font-size:22px;box-sizing:border-box;">

            <label style="display:block;font-size:32px;margin-bottom:14px;">📞 CONTACT:-</label>
            <input name="contact" required style="width:100%;padding:18px 20px;margin:0 0 30px 0;border-radius:6px;border:1px solid #777;font-size:22px;box-sizing:border-box;">
            ...
            <input name="paymentSS" type="file" accept="image/*" required style="width:100%;margin:0 0 42px 0;color:white;font-size:18px;">

            <button type="submit" style="width:100%;padding:22px 20px;font-size:28px;border:none;border-radius:6px;cursor:pointer;background:#e9e9e9;color:#111;font-weight:500;">
              Generate Ticket
            </button>
          </form>
...
app.post('/generate', upload.single('paymentSS'), async (req, res) => {
  try {
    ...
    ticket = await updateTicket(id, {
      ...
      mail_status: mailResult.success ? 'Sent' : `Failed: ${mailResult.error}`
    });
...
app.get('/verify/:id', async (req, res) => {
  try {
    ...
    if (!ticket) {
      return res.send(`
        <html><body style="text-align:center;background:#111;color:white;padding-top:50px;font-family:Arial;"><h1>❌ Invalid Ticket</h1></body></html>
      `);
    }
    ...
    const detailsBox = `
      <div style="display:inline-block;text-align:left;background:#1a1a1a;padding:25px;border-radius:12px;min-width:340px;">
        ...
        ${ticket.assigned_time ? `<p><strong>⏱ ASSIGNED AT:-</strong> ${htmlEscape(formatTimestamp(ticket.assigned_time))}</p>` : ''}
    ...
    if (ticket.assigned) {
      return res.send(`
        <html><body style="text-align:center;background:#111;color:white;padding-top:40px;font-family:Arial;"><h1>⚠️ Already Assigned</h1>${detailsBox}</body></html>
      `);
    }
      ...
      <html>
        <body style="text-align:center;background:#111;color:white;padding-top:40px;font-family:Arial;">
          ...
          <form method="POST" action="/assign/${ticket.id}">
            <button style="padding:12px 25px;font-size:16px;cursor:pointer;background:#4CAF50;color:white;border:none;border-radius:8px;">✅ Assign Entry</button>
          </form>
        </body>
      </html>
    `);
...
app.get('/resend-all', async (req, res) => {
  try {
    ...
    for (const existingTicket of tickets) {
      const newId = uuidv4();
      const verifyUrl = `${BASE_URL}/verify/${newId}`;
      const pdfBuffer = await createTicketPdfBuffer({
    ...
    console.error('ERROR IN RESEND-ALL:', err);
    res.status(500).send('Error: ' + err.message);
  }
});
...
app.get('/download-excel', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="ticket-data.xlsx"');
    res.send(Buffer.from(buffer));
  } catch (err) {
    console.error('DOWNLOAD EXCEL ERROR:', err);
    res.status(500).send('Error downloading Excel: ' + err.message);
  }
});
